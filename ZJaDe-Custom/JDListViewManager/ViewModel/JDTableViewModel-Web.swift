...
extension JDTableViewModel {
    ...
    func parseModelArray(_ modelArray:[JDTableModel]) {
        ...
        self.updateDataSource { (oldData) -> [(JDTableSection, [JDTableModel])]? in
            let section = oldData.last?.0 ?? JDTableSection()
            let models = (oldData.last?.1 ?? [JDTableModel]()) + modelArray
            return [(section,models)]
        }
    }
}
extension JDTableViewModel {// mj_header, mj_footer

...
class JDTableViewCell: UITableViewCell {
    ...
    func cellDidLoad(_ element: JDElementModel) {
        /// ZJaDe:horizontal
    ...
    final func cellWillAppear(_ element: JDElementModel) {
    ...
    func configCellWithElement(_ element: JDElementModel) {
    ...
    func cellUpdateConstraints(_ element: JDElementModel) {
        self.setNeedsUpdateConstraints()
    }
    // MARK: - cell已经消失,element解绑cell
    func cellDidDisappear(_ element: JDElementModel) {
        
    }
    // MARK: - 如果返回大于零，可以不用使用自动计算高度
    func calculateJDContentViewHeight(_ jdContentViewWidth:CGFloat,elementModel:JDElementModel) -> CGFloat {
        return 0
    }
}

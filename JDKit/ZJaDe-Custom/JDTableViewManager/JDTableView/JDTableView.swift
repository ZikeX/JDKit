...
class JDTableView: UITableView {
    ...
    let reloadDataSource = RxTableViewSectionedReloadDataSource<SectionModel<JDSection,JDElementModel>>()
    var dataArray = Variable([SectionModel<JDSection,JDElementModel>]())
    var page = 1
    ...
    func configDataSource() {
        ...
        _ = self.rx.willDisplayCell.subscribe { (event) in
            ...
            if let cell = element.cell as? JDTableViewCell,
                let model = try? self.rx.model(element.indexPath) as JDElementModel {
...
extension JDTableView:UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if let model = try? tableView.rx.model(indexPath) as JDElementModel {
            model.perform(#selector(JDElementModel.calculateCellHeight(_:)), on: Thread.main, with: self, waitUntilDone: false,modes:[RunLoopMode.defaultRunLoopMode.rawValue])
